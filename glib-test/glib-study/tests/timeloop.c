...
static void io_pipe(GIOChannel **channels) {
	int fds[2];
...
static gboolean read_all(GIOChannel *channel, char *buf, int len) {
	gsize bytes_read = 0;
	gsize count;
	GIOError err;
...
static gboolean write_all(GIOChannel *channel, char *buf, int len) {
...
static void run_child(GIOChannel *in_channel, GIOChannel *out_channel) {
	int i;
	int val = 1;
...
static gboolean input_callback(GIOChannel *source, GIOCondition condition, gpointer data) {
	int val;
...
static void create_child(void) {
	...
	if (pid > 0) /* Parent */
	{
		...
		g_io_add_watch(out_channels[0], G_IO_IN | G_IO_HUP, input_callback, in_channels[1]);
	} else if (pid == 0) /* Child */
	{
...
int main(int argc, char **argv) {
	...
	printf("Elapsed total: %g\n", difftimeval(&old_usage.ru_utime, &new_usage.ru_utime) + difftimeval(&old_usage.ru_stime, &new_usage.ru_stime));
	printf("total / iteration: %g\n",
			(difftimeval(&old_usage.ru_utime, &new_usage.ru_utime) + difftimeval(&old_usage.ru_stime, &new_usage.ru_stime)) / (n_iters * n_children));
