...
NCURSES_EXPORT(NCURSES_CONST char *) keyname (int c)
{
	...
	if (c == -1) {
		...
		if (result == 0 && (c >= 0 && c < SIZEOF_TABLE)) {
			if (keyname_table == 0)
				keyname_table = typeCalloc(char *, SIZEOF_TABLE);
			if (keyname_table != 0) {
				if (keyname_table[c] == 0) {
					int cc = c;
					p = name;
					if (cc >= 128) {
						strcpy(p, "M-");
					...
					if (cc < 32)
						...
						sprintf(p, "%c", cc);
					keyname_table[c] = strdup(name);
				}
				result = keyname_table[c];
			}
...
void _nc_keyname_leaks(void)
{
	...
	if (keyname_table != 0) {
		for (j = 0; j < SIZEOF_TABLE; ++j) {
			FreeIfNeeded(keyname_table[j]);
		}
		FreeAndNull(keyname_table);
	}
}
#endif /* NO_LEAKS */
